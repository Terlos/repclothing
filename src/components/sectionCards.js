...
export default function Card({items, productData, setType, type, setFilterData, selectedFilters, set, setTypeResult, leftInput, rightInput}){
    ...
    return(
      <div className="flex flex-row justify-between items-center w-4/5 gap-4">
        ...
        {items.map(({id, name, icon, filters})=>{
            return(
                <div key={id} onClick={() => filterHandler(filters)} className="flex flex-col justify-center items-center gap-1 hover:cursor-pointer">
